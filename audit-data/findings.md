...
**Description:** In the ThunderLoan system, the `exchangeRate` is responsible for calculating the exchange rate between assetTokens and underlying tokens. In a way, it's esponsible for keeping track of how many fees to give to liquidity providers. 

However, the `deposit` function updates this rate without collecting any fees. So, this update should be removed
...
Place this test function in the test file
...
**Recommended Mitigation:** Remove the incorrect updated exchange rate lines from the `deposit` function
